---
name: roadmap
description: Drive the Klimand tech roadmap. Phase C (plan/approve gate) is the only unshipped phase; bundled into /goal Phase 4 by default.
---

# Klimand Roadmap — `/roadmap`
...
- **Phase A** — shipped (cancel route + `registerAborter` wired in cli-tools and broker)
- **Phase B** — shipped (`web/lib/threads.ts` and thread list)
- **Phase C** — **NOT shipped** — the only remaining tech-roadmap item
- **Phase D** — shipped (`web/app/api/schedules/route.ts` + `[id]/route.ts`, live schedule records under `.agentchain/schedules/`)

If invoked without arguments, jump straight to Phase C. Phase C is also bundled into `/goal` Phase 4 (pre-launch polish); use whichever command you prefer to execute it — they'll edit the same files.
...
1. **Execute exactly one phase per `/roadmap` invocation.** Pick the lowest-letter phase not yet shipped. If the user asks for a specific phase, do that one.
...
3. **Pause and ask the user before starting the next phase.** Exception: if the user invoked you via `/loop /roadmap` or otherwise signalled "chain through all of them," continue without asking (per saved memory `feedback_loop_chained_phases`).
...
## Out of scope (do NOT do inside `/roadmap`)
...
3. Post a short summary to the user: which phase shipped, files changed, verification result. **Ask before starting the next phase** unless the user already signalled chain-mode (`/loop /roadmap` or "do all of them").
